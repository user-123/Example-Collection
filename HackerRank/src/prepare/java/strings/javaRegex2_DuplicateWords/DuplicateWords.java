...
public class DuplicateWords {
	...
	public static void main2(String[] args) {
		/* Enter your code here. Read input from STDIN. Print output to STDOUT. Your class should be named Solution. */
		String regex = "(?i)\\b(\\w+)\\b(?:\\s+\\1)+\\b";	/* Write a RegEx matching repeated words here. */
		Pattern p = Pattern.compile(regex	/* Insert the correct Pattern flag here.*/);	//參考：https://www.hackerrank.com/challenges/duplicate-word/forum
		...
		while (numSentences-- > 0) {
			...
			// Check for subsequences of input that match the compiled pattern
			while (m.find()) {
				input = input.replaceAll(regex	/* The regex to replace */, "$1"	/* The replacement. */);
			}
