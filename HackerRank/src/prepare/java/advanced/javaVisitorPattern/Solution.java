package prepare.java.advanced.javaVisitorPattern;
...
class ProductOfRedNodesVisitor extends TreeVis {
	...
	private void muiltply(Tree tree) {
		productOfRedNodes *= tree.getColor()==Color.RED ? tree.getValue() : 1;
		productOfRedNodes %= MODULO;
	}
...
class FancyVisitor extends TreeVis {
	...
	public void visitNode(TreeNode node) {
		//implement this
		sumOfValuesNonLeafEvenDepth += node.getDepth()%2==0 ? node.getValue() : 0;
	}

	public void visitLeaf(TreeLeaf leaf) {
		//implement this
		sumOfValuesGreenLeaf += leaf.getColor()==Color.GREEN ? leaf.getValue() : 0;
	}
}
...
public class Solution {
	...
	public static Tree solve() {
		...
		try(Scanner scanner = new Scanner(System.in)) {
			...
			int[][] nodesAttributes = new int[quantity][2];
			for(int i=0; i<quantity; i++) {
				nodesAttributes[i][0]=scanner.nextInt();
			}
			for(int i=0; i<quantity; i++) {
				nodesAttributes[i][1]=scanner.nextInt();
			}
			List<List<Integer>> childrenList = new ArrayList<>(quantity);
			for(List<Integer> children : childrenList) {
				children=new ArrayList<>();
			}
			while(scanner.hasNext()) {
				childrenList.get(scanner.nextInt()-1).add(scanner.nextInt()-1);
			}
			Map<Integer, Tree> treeMap = new HashMap<>();
			for(int i=0; i<childrenList.size(); i++) {
				//if(childrenList.get(i).size()==0) {treeMap.put(0, new TreeLeaf(nodeAttribute[i][0], nodeAttribute[i][1]==0 ? Color.RED : Color.GREEN, 00000000000));}
			}
		}
		return new TreeLeaf(0, null, 0);
	}

	private static void DFS(int n, Map<Integer, ArrayList<Integer>> edges, int[][] nodesAttributes) {
		boolean[] visited = new boolean[n+1];
		TreeNode parent = (TreeNode)tree.get(1);
		DFSUtil(parent, 1, visited, edges, nodesAttributes);
	}

	private static void DFSUtil(TreeNode parent, int v, boolean[] visited, Map<Integer, ArrayList<Integer>> edges, int[][] nodesAttributes) {
		visited[v] = true;
		...
		while(iterator.hasNext()) {
			int n=iterator.next();
			if(!visited[n]) {DFSUtil(treeNode, n, visited, edges, nodeAtts);}
		}
	}
